...
def shortest_path(source, target):
    ...
    while not frontier.empty():
        parent_node = frontier.remove() # pops out the first node

        # Check if the node has already been explored
        if parent_node.state in explored:
            continue

        # check if goal state found
        if parent_node.state == target:
            # Found the target, now trace back the path
            path = []
            current = parent_node
        ...
        explored.add(parent_node.state)  

        children = neighbors_for_person(parent_node.state) # expand child node
        for movie_id, person_id in children:
            # pair[0] = movie_id, pair[1] = person_id
            if person_id in explored or frontier.contains_state(person_id):
                continue

            child_node = Node(person_id, parent_node, movie_id)
